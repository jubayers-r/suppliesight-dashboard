...
async function startServer() {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    // Calculate total stock and demand for this date (with some variation)
    const variation = 0.95 + Math.random() * 0.1; // 95% to 105% of current values
    const totalStock = Math.round(products.reduce((sum, p) => sum + p.stock, 0) * variation);
    const totalDemand = Math.round(products.reduce((sum, p) => sum + p.demand, 0) * variation);
  ...
  Mutation: {
    ...
    transferStock: (_, { id, from, to, qty }) => {
      ...
      if (!sourceProduct || sourceProduct.stock < qty) {
        throw new Error("Invalid transfer: insufficient stock or product not found");
      }
